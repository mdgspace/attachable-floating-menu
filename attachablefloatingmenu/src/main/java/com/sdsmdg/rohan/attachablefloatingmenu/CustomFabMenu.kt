...
class CustomFabMenu @JvmOverloads constructor(
        context: Context,
        val startX: Float = -1f,
        val startY: Float = -1f,
    ...
    companion object {
        const val LOG_TAG = "CustomFabMenu"
    }
    ...
    init {
        ...
        addView(CustomFab(context), 0)
        addView(CustomFab(context), 1)
        val smallView = CustomFab(context)
        smallView.fabSize = CustomFab.FabSize.SIZE_MINI
        addView(smallView, 2)
    }
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        for (index in 0 until childCount) {
            val child = getChildAt(index) as CustomFab
    ...
    private fun animateMe() {
        for (i in 0 until childCount) {
            val child = getChildAt(i) as CustomFab
